...
@section('content')
    <div class="all-wrapper menu-side with-pattern">
        <div class="auth-box-w wider">
            ...
            <h4 class="auth-header">Registre-se agora!</h4>
            <form method="POST" action="{{ route('register') }}">
                @csrf
                <div class="form-group">
                    <label for=""> Seu Nome</label>
                    <div class="pre-icon os-icon os-icon-email-2-at2"></div>
                    <input id="name" type="name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" placeholder="Nome" value="{{ old('name') }}" required autofocus>
                    @if ($errors->has('name'))
